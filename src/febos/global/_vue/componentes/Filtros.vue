<template>
  <div>
    <div style="float:left">
      <vs-chip :color="colores.navbar" v-if="filtrosAplicados.length > 0" class="accionable"
      ...
      <vs-dropdown>
        ...
        <vs-dropdown-menu>
          <vs-dropdown-item v-for="(filtro, index) in filtrosDisponibles" :key="`filtro-${index}`"
                            v-on:click.native="agregarFiltro(filtro,true)" style="width:200px">
    ...
    <vs-chip :color="colores.navbar" v-for="(filtro,index) in filtrosAplicados" :key="`filtro-aplicado-${index}`"
      ...
      <vs-tooltip :text="`Eliminar filtro de ${filtro.nombre}`">
        <vs-avatar icon="clear" color="primary" v-on:click.native="eliminarFiltro(filtro)"  v-if="esEliminable[filtro.campo]"/>
        <span>&nbsp;</span>
      </vs-tooltip>
      <vs-tooltip :text="`Modificar filtro ${filtro.nombre}`">
        <strong>{{ typeof filtro == 'undefined' ? '' : filtro.nombre }}: </strong><span
      class="pl-1">{{ typeof filtro == 'undefined' ? '' : filtro.valorFormateado || "" }}</span>
      </vs-tooltip>
    </vs-chip>
    ...
    <vs-modal ref="configFiltro" v-on:close="verificarFiltrosAlCerrar">
      ...
      <div v-if="filtroActual.tipo == 'multi'">
        <div v-if="filtroActual.valor.length==1" class="alerta">
          Debe seleccionar como mínimo 1 de las opciones. Seleccione otra opcion antes de desmarcar la actual si es que
          desea cambiarla.
        </div>
        ...
        <div v-for="opcion in filtroActual.opciones" v-bind:key="opcion.nombre" class="modal-opcion">
          <vs-checkbox v-model="filtroActual.valor" :vs-value="parseInt(opcion.valor)"
      ...
      <div v-if="filtroActual.tipo == 'rangoFecha'">
        ...
        <div v-if="!tipoRangoFechaAvanzado">
          <div class="center">
            <template v-if="periodo.valor != 'personalizado'" v-for="(periodo, index) in periodos">
              <vs-button :disabled="periodo.valor == filtroActual.valor" size="medium" color="primary" type="filled"
                         v-on:click="seleccionarRango(filtroActual,periodo.valor)" class="w-full mb-1"
                         :key="`periodo-${index}`">
        ...
        <div v-if="tipoRangoFechaAvanzado">
          entre el
          <datetime v-model="rangoAvanzado.desde" input-class="fecha" :phrases="{ok: 'Seleccionar', cancel: 'Cancelar'}"
                    value-zone="local" format="yyyy-MM-dd"></datetime>
          y el
          <datetime v-model="rangoAvanzado.hasta" input-class="fecha" :phrases="{ok: 'Seleccionar', cancel: 'Cancelar'}"
                    value-zone="local" format="yyyy-MM-dd"></datetime>
        </div>
      </div>
    </vs-modal>
...
export default {
  ...
  computed: {
    ...
    esEliminable(){
      let tipoFiltros={};
      this.configuracionVista.filtrosHabilitados.forEach(function(filtro){
        tipoFiltros[filtro.campo]=true;
      });
      this.configuracionVista.filtrosPorDefecto.forEach(function(filtro){
    ...
    filtrosDisponibles() {
      try {
        ...
        console.log("filtros habilitados", JSON.parse(JSON.stringify(filtros)));
        return filtros;
      } catch (e) {
        console.log(e);
  ...
  methods: {
    ...
    aplicarFiltros() {
      let query = [];
      let that = this;
      this.filtrosAplicados.forEach(function (filtro) {
        if (filtro.tipo == 'rangoFecha' && !filtro.valor.includes("--")) {
          let inicio = that.formatoTipoRango(filtro.valor);
          let fin = Vue.moment().subtract(0, 'days').format('YYYY-MM-DD');
      ...
      this.configuracionVista.filtrosFijos.forEach(function (filtro) {
      ...
      switch (filtro.tipo) {
        ...
        case "multi": {
          ...
          filtro.valor.forEach(
            valor => {
              console.log("filtro habilitado",filtroHabilitado);
        ...
        case "numero": {
          ...
          if (filtro.valor.includes("--")) {
            ...
            if (filtro.valor.length > 7) {
              let nuevoArreglo = filtro.valor.slice(0, 7);
    ...
    agregarFiltro(filtro,desplegarVentanaDeModificacion=false) {
      filtro = this.formatearValor(filtro);
      console.log("agregando filtro",JSON.parse(JSON.stringify(filtro)));
