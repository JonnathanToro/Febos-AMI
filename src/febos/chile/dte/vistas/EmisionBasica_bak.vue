...
export default {
  ...
  data() {
    return {
      ...
      referencia: {
        formatFecha: null, fecha: null, motivo: null, folio: null, tipoDocumento: null
      },
      referencias: [],
      tiposDocumento: [
  ...
  methods: {
    ...
    enviarDocumento() {
      ...
      const rutEmpresa = storage.Empresas.empresa.iut;
      const { token } = storage.Usuario;
      const url = `https://api.febos.cl/${ process.env.VUE_APP_AMBIENTE }/empresas/${ rutEmpresa}`;
      const headersEmpresa = {
        token
      };

      // OBTENIENDO DATOS EMPRESA DESDE API (IDEAL LOCAL)
      axios.get(url, { headers: headersEmpresa }).then((response) => {
        const f2String = this._generarF2(response.data);
        const headersEmitir = {
          token,
          empresa: rutEmpresa
        };
        ...
        const url = `https://api.febos.cl/${ process.env.VUE_APP_AMBIENTE }/documentos`;
        axios
          .post(url, bodyParams, {
    ...
    _generarF2(data) {
      ...
      const rutEmpresa = data.iut;
      const razonSocialEmpresa = data.razonSocial;
      const giroEmpresa = data.giro;
      const sucursalMatriz = data.sucursales.filter((sucursal) => sucursal.casaMatriz)[0];
      const telefonoEmpresa = sucursalMatriz.telefono;
      const correoEmpresa = sucursalMatriz.correoElectronico;
      const { acteco } = data;
      const direccionEmpresa = sucursalMatriz.direccion;
      const comunaEmpresa = sucursalMatriz.comuna;
      const ciudadEmpresa = sucursalMatriz.ciudad;

      // DATOS RECEPTOR
      const { rutReceptor } = this.encabezado;
      const razonSocialReceptor = this.encabezado.razonSocial;
      const giroReceptor = this.encabezado.giro;
      const direccionReceptor = this.encabezado.direccion;
      const comunaReceptor = this.encabezado.comuna;
      const ciudadReceptor = this.encabezado.ciudad;
      const emailReceptor = this.encabezado.email;
      ...
      const size = this.detalle.length;
    ...
    abrirReferencia() {
      this.referencia = {
        fecha: null, motivo: null, folio: null, tipoDocumento: null
      };
